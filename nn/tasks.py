...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor): The last output of the model.
        """
        # # 如果输入是 6 通道，这里把它切成 [RGB张量, IR张量] 的列表
        ...
        for m in self.model:
            ...
            # print(f"layer {m.i}: {m.__class__.__name__}")
            # if isinstance(x, torch.Tensor):
            #     print(" input:", x.shape)
            # elif isinstance(x, list):
            #     print(" input list:", [t.shape for t in x])
            x = m(x)  # run
            # if isinstance(x, torch.Tensor):
            #     print("output:", x.shape)
            # elif isinstance(x, (list, tuple)):
            #     print("output list:", [t.shape for t in x if hasattr(t, 'shape')])
            # print("-" * 60)
    ...
    def loss(self, batch, preds=None):
        ...
        # # 这里调用切分函数，把 batch["img"] 从 6通道张量 变成 [张量1, 张量2]
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # parse_model 函数会遍历 yaml 里的 backbone 和 head，一层层实例化 PyTorch 模块
        # self.model: 这是一个 nn.Sequential，里面存了网络的所有层
        # self.save: 这是一个列表，记住了哪些层的输出需要被“暂存” (用于后面的 Concat 或融合)
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)  # model, savelist
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}  # default names dict
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)

        # Build strides  YOLO 是下采样网络，需要知道图片经过网络后缩小了多少倍 (通常是 32 倍)
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 256  # 2x min stride
            ...
            def _forward(x):
                ...
                def unwrap(o):
                    ...
                    if isinstance(o, (list, tuple)):
                        return unwrap(o[0])
                    raise TypeError(f"Unexpected type in forward: {type(o)}")
                return unwrap(output)
            m._in_stride_init = True
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))])  # forward
            m._in_stride_init = False
    ...
    def init_criterion(self):
        ...
        from ultralytics.utils.loss import v8DetectionLoss, E2EDetectLoss, MultiLoss, DKDNetLoss, MultiOBBLoss
        #  检查最后一层（检测头）的类型
        # self.model[-1] 是网络的最末端
        m = self.model[-1]
        if getattr(self, "end2end", False):
            print(">>> using E2EDetectLoss")
            return E2EDetectLoss(self)
        elif m.__class__.__name__ == "DetectV5":
            print(">>> using DetectLossV5")
            device = next(self.parameters()).device
            if not hasattr(m, 'stride') or len(m.stride) == 0:
                print( ">>> Manually setting stride for DetectV5")
                m.stride = torch.tensor([4., 8., 16.], device=device)
            # 同时需要根据步长缩放 Anchors
            # 注意：只需在 stride 第一次赋值时执行一次
                if hasattr(m, 'anchors'):
                    m.anchors /= m.stride.view(-1, 1, 1)
            return DetectLossV5(self)
        # 如果是你的多分支检测头，使用 MultiLoss
        elif m.__class__.__name__ == "MultiDetect":
            print(">>> using MultiLoss")
            return MultiLoss(self)
        elif m.__class__.__name__ == "MultiOBBDetect":
            print(">>> use MultiOBBLoss，")
            return MultiOBBLoss(self, use_distillation=True)
        else:
            has_bdfusion = any(x.__class__.__name__ == "BDFusion" for x in self.model.modules())
            if has_bdfusion:
                print(">>> using DKDNetLoss")
                return DKDNetLoss(self)
            # 否则使用原生的 YOLO 损失函数
            print(">>> using default v8DetectionLoss")
...
class OBBModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from ultralytics.utils.loss import v8OBBLoss, MultiOBBLoss
        m = self.model[-1]  # 获取检测头

        # 如果使用的是你自定义的多分支旋转框检测头
        if m.__class__.__name__ == "MultiOBBDetect":
            # 返回支持蒸馏和多路损失计算的 MultiOBBLoss
            print(">>> 成功挂载：MultiOBBLoss (支持多路检测与蒸馏) <<<")
            return MultiOBBLoss(self, use_distillation=True)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in frozenset({Detect, DetectV5, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}):
            ch_list = [ch[x] for x in f]
            # args.append([ch[x] for x in f])
            if m is OBB:
                # OBB 需要三个参数 (nc, ne, ch)
                args = [args[0], 1, ch_list]
            elif m is DetectV5:
                args = [args[0], args[1], ch_list]
            else:
                # 其他 Head 维持原样
                args.append(ch_list)
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, DetectV5}:
                m.legacy = legacy
        ...
        # 可以引入新的
            ...
            c1 = [ch[x] for x in f]  # 去ch检查输入层f分别为多少通道
            c2 = c1[0]  # 假设输和输入的第一层通道一致
            args = [c2, *args]
        elif m is TwoFreDS:
            c1 = [ch[x] for x in f]
            c2 = make_divisible(args[0] * width, 8)*2
            args = [c2//2, *args[1:]]
        elif m is MultiDetect:
            c1 = [ch[x] for x in f]  # 提取这 9 层的通道数，组成一个列表
            args = [nc, c1]  # 重新包装参数：类别数和通道列表
            c2 = None  # 实例化 MultiDetect
        elif m is MultiOBBDetect:
            c1 = [ch[x] for x in f]
            args = [args[0], 1, c1]
            c2 = None
        # 复现论文CMAFF
        ...
        if i == 0:
            ch = []
        ch.append(c2)
    # print(f"Layer {i} ({m}): output_ch = {c2}, all_ch_list = {ch}")
