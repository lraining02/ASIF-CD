...
class SPPF(nn.Module):
    ...
    用极低的计算成本，显著扩大感受野（Receptive Field），让模型既能看清局部细节，也能理解全局概貌"""
...
class C2f(nn.Module):
    ...
    CSP 跨阶段局部网络”结构（即：把数据劈成两半，一半去卷，一半直接连过去，最后拼起来）"""
...
class FeatureAdd(nn.Module):
    """
    直接将多个输入特征图进行逐元素相加。
    要求输入特征图的 Shape (B, C, H, W) 完全一致。
    """
    def __init__(self, *args, **kwargs):
        super().__init__()
    def forward(self, x):
        # x 是一个列表，包含需要相加的特征图 [feat1, feat2, ...]
...
class DDConv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=1, g=1, act=True):
        ...
        self.c2 = c2
        # 偏移量生成：2 * k * k (x, y方向)
        self.offset_conv = nn.Conv2d(c1, 18, kernel_size=k, stride=s, padding=p)
        # 扩散掩码生成：k * k (用于自适应长距离感知)
        self.mask_conv = nn.Conv2d(c1, 9, kernel_size=k, stride=s, padding=p)
        # 实际的卷积权重
        self.weight = nn.Parameter(torch.Tensor(c2, c1 // g, k, k))
        self.bias = nn.Parameter(torch.Tensor(c2))
        nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        nn.init.zeros_(self.bias)
        nn.init.zeros_(self.offset_conv.weight)  # 初始偏移为0
        nn.init.zeros_(self.offset_conv.bias)
        nn.init.zeros_(self.mask_conv.weight)  # 初始掩码中间值
        nn.init.zeros_(self.mask_conv.bias)
        self.gn = nn.GroupNorm(num_groups=1, num_channels=c2) # 论文中提到的标准化

    def forward(self, x):
        offset = self.offset_conv(x)
        mask = torch.sigmoid(self.mask_conv(x))
        # 使用 torchvision 的 deform_conv2d 算子
        x = deform_conv2d(x, offset, self.weight, self.bias, stride=(1,1), padding=(1,1), mask=mask)
        return self.gn(x)
