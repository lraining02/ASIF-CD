...
class AdaptiveLightMasking(BaseTransform):
    """
    C2DFF-Net: 自适应光照感知掩码 (ALM)
    集成至 Ultralytics 框架，支持双流 (RGB + IR) 图像处理。
    """

    def __init__(self, n=20, k=10, p=5, prob=0.3):
        super().__init__()
        self.n = n  # 网格数量 (N x N)
        self.k = k  # 候选补丁数量 (K)
        self.p = p  # 在RGB亮区掩盖的数量 (P)
        self.prob = prob  # 触发概率
    ...
    def __call__(self, labels):
        ...
        img = labels["img"]  # 获取图像数据
        h, w, c = img.shape

        # 判定是否为双流数据 (假设前3通道为RGB，后3通道为IR，或根据你的逻辑调整)
        if c < 6:
            return labels
        ...
        for i in range(self.n):
            for j in range(self.n):
                patch = img_vis[i * patch_h:(i + 1) * patch_h, j * patch_w:(j + 1) * patch_w]
                brightness.append(np.mean(patch))

        # 2. 排序并确定暗区和亮区
        sorted_indices = np.argsort(brightness)
        dark_indices = sorted_indices[:self.k].tolist()
        bright_indices = sorted_indices[-self.k:].tolist()

        # 3. 执行非对称掩码策略
        # 暗区策略: 遮蔽可见光，强制网络从红外特征中学习
        for idx in dark_indices:
            r, c_idx = divmod(idx, self.n)
            img_vis[r * patch_h:(r + 1) * patch_h, c_idx * patch_w:(c_idx + 1) * patch_w] = 0

        # 亮区策略: 模拟眩光/过曝
        vis_mask_indices = random.sample(bright_indices, self.p)
        for idx in bright_indices:
            r, c_idx = divmod(idx, self.n)
            if idx in vis_mask_indices:
                img_vis[r * patch_h:(r + 1) * patch_h, c_idx * patch_w:(c_idx + 1) * patch_w] = 0
            else:
                img_ir[r * patch_h:(r + 1) * patch_h, c_idx * patch_w:(c_idx + 1) * patch_w] = 0

        # 合并回原图
        labels["img"] = np.concatenate([img_vis, img_ir], axis=-1)
        return labels
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            if img.shape[-1] > 5:  # judgment of RGB or RGB+IR
                ...
                img[..., -3:] = cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR)  # 写回后3通道
                img[..., :-3] = cv2.cvtColor(im_hsv_, cv2.COLOR_HSV2BGR) # 写回前3通道
                labels["img"] = img
            else:
                hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2HSV))
                im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose(
        [
            ...
            # 插入 ALM 增强
            AdaptiveLightMasking(n=20, k=10, p=5, prob=0.0),
        ]
    )  # transforms
