...
class TaskAlignedAssigner(nn.Module):
    """
    正负样本筛选时，同时考虑定位和分类得分，
    定义了一个 对齐指标 (Alignment Metric)：
    只有那些分类分数高且定位（IoU）准的锚点（Anchor Points），才会被选为正样本
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        代码计算了一个得分 align_metric
        对于通过初筛的锚点，计算它们相对于每个 GT 的“对齐指标”"""
        ...
        # alpha ($0.5$), beta ($6.0$)：超参数。可
        # 以看到 beta很大，说明 TAL 非常看重 IoU（定位质量）。
        # 如果框不准，哪怕分类分再高，得分也会被狠狠压低。
    ...
    # 对于每一个 GT，只要它得分最高的k个锚点（默认为 13 个，由 topk 参数控制）
    ...
    def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9):
        ...
        Note:
            ...
            算中心点到 GT 四条边的距离 (bbox_deltas)，如果四个距离都大于 0，说明在框内。
        """
    ...
    @staticmethod # 如果一个锚点同时被两个 GT 选中（比如两个物体靠得很近），归IOU更大的
