...
# ciou 它不仅看重叠面积，还看中心点距离，更看长宽比是否一致
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            # rho2: 中心点距离平方 (公式里的 /4 是因为用了直径而不是半径计算)
            rho2 = (
                (b2_x1 + b2_x2 - b1_x1 - b1_x2).pow(2) + (b2_y1 + b2_y2 - b1_y1 - b1_y2).pow(2)
            ) / 4  # center dist**2
            # 加入了长宽比惩罚项 它希望预测框的长宽比 $(w/h)$ 尽可能接近真实框的长宽比
...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, gt_bboxes, gt_cls):
        ...
        # )
        is_obb = (self.task == 'obb') or (gt_bboxes.shape[1] == 5)

        if is_obb:
            # 如果 detections 还是 67 维原始数据，说明后处理没做完
            # 这里必须确保 detections 已经通过了解码，否则 batch_probiou 也会报错
            # 如果此时 detections 依然是 67 维，我们需要手动切出前 4 位和最后 1 位
            if detections.shape[1] > 7:
                # 这是一个临时补丁，建议还是修复验证器
                preds = torch.cat([detections[:, :4], detections[:, -1:]], dim=-1)
            else:
                preds = torch.cat([detections[:, :4], detections[:, -1:]], dim=-1)

            iou = batch_probiou(gt_bboxes, preds)
        else:
