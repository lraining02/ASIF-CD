...
def plot_images(
    ...
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # 计算当前图片在拼图中的起始坐标 [cite: 126]

        #  调整维度顺序从 [C, H, W] 变为 [H, W, C]
        img_to_plot = images[i].transpose(1, 2, 0)

        #  关键修复：如果输入是 6 通道（光学+SAR），强制只取前 3 个通道（RGB 光学部分）[cite: 112, 128]
        if img_to_plot.shape[2] > 3:
            img_to_plot = img_to_plot[:, :, :3]
        #  将处理后的 3 通道图片填入画布相应位置 [cite: 111, 219]
        mosaic[y: y + h, x: x + w, :] = img_to_plot
